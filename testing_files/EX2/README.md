...
| `error-2.xml` | Faulty: **Avrum's `initial-cash` is 0**, and the spec requires it above 0. Everything else is the content of `multiple.xml`, so the file is refused for that one reason and no other. |
